...
#define NOR_SCAN_PER      0xEE // read-write. Normal fast detection cycle. This value affects LpAutoWakeTime and AutoSleepTime. Unit 10ms, possible value: 1～30. The default value is 1.
#define MOTION_S1_ANGLE   0xEF // read-write. Gesture detection sliding partition angle control. Angle=tan(c)*10. c is the angle based on the positive direction of the x-axis.
...
#define LP_AUTO_WAKE_TIME 0xF4 // read-write**. Automatic recalibration cycle in low power mode. The unit is 1 minute, and the possible value is 1 to 5. The default value is 5.
#define LP_SCAN_TH        0xF5 // read-write**. Low-power scan wake-up threshold. The smaller the more sensitive. Possible values: 1 to 255. The default value is 48.
#define LP_SCAN_WIN       0xF6 // read-write**. Low power scan range. The larger the more sensitive, the higher the power consumption. Possible values: 0, 1, 2, 3. The default value is 3.
#define LP_SCAN_FREQ      0xF7 // read-write**. Low power scan frequency. The smaller the more sensitive. Possible values: 1 to 255. The default value is 7.
#define LP_SCAN_I_DAC     0xF8 // read-write**. Low power scan current. The smaller the more sensitive. Possible values: 1 to 255.
#define AUTO_SLEEP_TIME   0xF9 // read-write**. Automatically enter low power mode when there is no touch for x seconds. The unit is 1s, the default value is 2.

#define IRQ_CTL           0xFA // read-write*. Interrupt configuration. 0x60 = report mode, 0x11 = gesture mode, 0x71 = both.
#define AUTO_RESET        0xFB // read-write. Automatic reset (cancel) when there is a touch but no valid gesture within x seconds. The unit is 1s. When it is 0, this function is not enabled. Default is 5.
#define LONG_PRESS_TIME   0xFC // read-write. Automatic reset (cancel) after long press for x seconds. The unit is 1s. When it is 0, this function is not enabled. Default is 10.
#define IO_CTL            0xFD // read-write. Pin IO configuration.
#define DIS_AUTO_SLEEP    0xFE // read-write**. The default is 0, enabling automatic entry into low power mode. When it is a non-zero value, automatic entry into low power mode is disabled.
