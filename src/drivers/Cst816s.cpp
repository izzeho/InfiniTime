...
bool Cst816S::Init() {
  ...
  static constexpr uint8_t motionMask = 
    MOTION_MASK_EN_DCLICK | MOTION_MASK_EN_CON_UD | MOTION_MASK_EN_CON_LR;
  twiMaster.Write(twiAddress, MOTION_MASK, &motionMask, 1);

  // Configure interrupt generating events
  static constexpr uint8_t irqCtl = 
    IRQ_CTL_EN_MOTION | IRQ_CTL_EN_CHANGE | IRQ_CTL_EN_TOUCH;
...
Cst816S::TouchInfos Cst816S::GetTouchInfo() {
  ...
  if (ret != TwiMaster::ErrorCodes::NoError) return info;
  ...
  info.isValid = (info.x < maxX && info.y < maxY &&
    (info.gesture == Gestures::None || info.gesture == Gestures::SlideDown ||
    info.gesture == Gestures::SlideUp || info.gesture == Gestures::SlideLeft ||
    info.gesture == Gestures::SlideRight || info.gesture == Gestures::SingleTap ||
